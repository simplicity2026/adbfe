...
void RunCommandAsync(const std::string& cmd, bool HideProcess);
...
std::string BrowseForFile(std::string extensions = "");
...
bool SW1rmwpCB1 = true, SW1epciscCB1 = true, SW1rwlbCB1 = true, SW1sbofdiCB1 = true, SW1cbpCB1 = true, SW1usctoiCB1 = false;
bool FUNC2Running = false;

void MBD()
{
	MessageBoxA(NULL,"Debug",APP_TITLE2,MB_OK|MB_ICONINFORMATION);
}
...
void DrawListViewCell(HDC hdc, RECT rc, int row, int col, bool selected, bool reverse)
{
	...
	switch (col) {
	case 0: {
		RECT cb = { rc.left + 7, rc.top + 4, rc.left + 19, rc.top + 16 };
	...
	case 2: {
		...
		DrawIconEx(hdc, rc.left + 6, rc.top + 1, hIcon, 16, 16, 0, NULL, DI_NORMAL);
		DestroyIcon(hIcon);
		...
		DrawTextA(hdc, f.Name.c_str(), f.Name.length(), &textRect,
	...
	case 3: {
		...
		if (f.Type != "FOLDER") type = DefineFileType(f.Name).c_str();
	...
	case 4: {
		if (f.Type == "FOLDER")
			...
			strcpy(buf, f.FormattedSize.c_str());
	...
	case 5:
		DrawTextA(hdc, f.DateTime.c_str(), f.DateTime.length(), &rc,
...
void RunCommandAsync(const std::string& cmd, bool HideProcess)
{
	std::thread([cmd, HideProcess]() {
		if (HideProcess) {
			// Hide the window (not recommended for debugging)
...
void OpenItem(const std::string& androidPath, const std::string& Type, long long SizeInBytes)
{
	...
	std::string localFilePath = dateFolder + fileName;
	std::string pullCmd = adbPath + " pull -p \"" + androidPath + "\" \"" + localFilePath + "\"";

	int result = system(pullCmd.c_str());
...
std::string BrowseForFile(std::string extensions)
{
	OPENFILENAME ofn;
	char szFile[260];
	static std::string filter; // Make filter static
	...
	if (extensions.empty()) {
		...
		filter = "Supported Files ("; // Assign to static filter
		std::string filterExtensions;
		size_t pos = 0;
		while ((pos = extensions.find(",")) != std::string::npos) {
			std::string ext = extensions.substr(0, pos);
			filterExtensions += "*." + ext + ";";
			extensions.erase(0, pos + 1);
		}
		filterExtensions += "*." + extensions;
		filter += filterExtensions + ")\0" + filterExtensions + "\0All Files\0*.*\0";
...
void TryDownloading(void)
{
	...
	*/
	...
	for (size_t i = 0; i < paths.size(); ++i) {
		...
		RunCommandAsync(pullCmd, false);
	}

	std::string RunWithExplorerCMD = "explorer \"" + destDir + "\"";
	system(RunWithExplorerCMD.c_str());
...
void TryDeleting(void)
{
	...
	if (paths.size() == 1 && paths[0] == "/sdcard/") {
		MessageBoxA(NULL, "Cannot delete /sdcard/", "Error", MB_OK | MB_ICONERROR);
		return;
	}
...
void SaveMainWindowSettings(void)
{
	...
	if(!AllowSaveMainWindowSettings || !SW1rmwpCB1) {
		LOG_THIS("[ void SaveMainWindowSettings() ] Attempting to SaveMainWindowSettings...\r\n");
		return;
	}
...
void LOG_THIS(std::string content)
{
	if (content.empty() || content.length() < 5) return;
...
void PRELOADFL(void)
{
	...
	if (file_exists("adb.exe")) {
		adbPath = "adb.exe";
	} else if (file_exists("G:\\scrcpy\\adb.exe")) {
		adbPath = "G:\\scrcpy\\adb.exe";
	}
	
	std::string adbPath_reg = GRSS("adbPath", "NONE");
	if(adbPath_reg != "NONE" && file_exists(adbPath_reg)) { adbPath = adbPath_reg; }

	// Handle location of adb.exe
	...
	if (adbPath.empty()) {
		...
		if (step1 == IDYES) {
			std::string filePath = BrowseForFile("exe");
	...
	// Check if adb.exe is connected to any device
...
void StartFEWindow(HINSTANCE hInst, int nCmdShow)
{
	...
	if(SW1rmwpCB1) {
		height = std::stoi(GRSS("OnStartUpHeight", "405"));
		width = std::stoi(GRSS("OnStartUpWidth", "665"));
...
void StartSettingsWindow1(void)
{
	...
	std::string adbPath_reg = GRSS("adbPath", "NONE");
	if(adbPath_reg != "NONE" && file_exists(adbPath_reg)) { adbPath = adbPath_reg; }
	if(GRSS("RestoreMainWindowPosition", "Yes") == "No") { SW1rmwpCB1 = false; }
	if(GRSS("EscapeADBShellCommand", "Yes") == "No") { SW1epciscCB1 = false; }
	if(GRSS("RewriteTEXT2WindowLinesBreaks", "Yes") == "No") { SW1rwlbCB1 = false; }
	if(GRSS("BackUpDeletedItems", "Yes") == "No") { SW1sbofdiCB1 = false; }
	if(GRSS("CacheFileLists", "Yes") == "No") { SW1cbpCB1 = false; }
	if(GRSS("UseSingleClickInFileList", "Yes") == "No") { SW1usctoiCB1 = false; }
	...
	hSettingsWindow1DefaultBTN1 = CreateWindowEx(
	                                  0, "BUTTON", "Defaults", WS_VISIBLE | WS_CHILD,
	...
	if (SW1rmwpCB1) { SendMessage(hSW1rmwpCB1, BM_SETCHECK, BST_CHECKED, 0); }
	...
	if(SW1epciscCB1) { SendMessage(hSW1epciscCB1, BM_SETCHECK, BST_CHECKED, 0); }
	...
	if (SW1rwlbCB1) { SendMessage(hSW1rwlbCB1, BM_SETCHECK, BST_CHECKED, 0); }
	...
	if (SW1sbofdiCB1) { SendMessage(hSW1sbofdiCB1, BM_SETCHECK, BST_CHECKED, 0); }
	...
	if(SW1cbpCB1) { SendMessage(hSW1cbpCB1, BM_SETCHECK, BST_CHECKED, 0); }
	...
	if(SW1usctoiCB1) { SendMessage(hSW1usctoiCB1, BM_SETCHECK, BST_CHECKED, 0); }
...
LRESULT CALLBACK hListViewWindow1Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: {
		...
		SetTimer(hListViewWindow1, 3, 500, Timer3Proc);
		...
		HIMAGELIST hDummyImageList = ImageList_Create(1, 19, ILC_COLOR32, 1, 1);
...
LRESULT CALLBACK CustomListView1Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSELEAVE: {
		static bool g_trackingMouse = true;
...
LRESULT CALLBACK SettingsWindow1Proc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		else if (LOWORD(wParam) == 2 && !FUNC2Running)
		{
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (hwndExisting != NULL) {
		...
		ShowWindow(hwndExisting, SW_RESTORE); // Restore if minimized
